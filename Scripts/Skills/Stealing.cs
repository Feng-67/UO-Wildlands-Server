...
namespace Server.SkillHandlers
{
    ...
    {
        Mobile rootCheck = target as Mobile;
        if (rootCheck == null && target is Item)
            rootCheck = ((Item)target).RootParent as Mobile;

        if (rootCheck != null && rootCheck.Player)
        {
            from.SendMessage("Stealing from other players is disabled");
            return;
        }
    }
    		// END OF FIX
