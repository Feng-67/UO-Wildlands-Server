...
// INSERT THIS BLOCK: Explicitly block players from snooping other players
    if (from.Player && to.Player && from.AccessLevel == AccessLevel.Player)
    {
        from.SendMessage("Snooping other players is disables");
        return false;
    }

    Map map = from.Map;

    // The rest of the logic handles NPCs and Felucca rules for NPCs
    if (to.Player)
