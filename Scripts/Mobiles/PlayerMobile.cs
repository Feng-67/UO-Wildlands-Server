...
namespace Server.Mobiles
{
	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
        ...
        // Fixed: Removed the duplicate property and kept only one.
        public SkillName[] AnimalFormRestrictedSkills => m_AnimalFormRestrictedSkills;
        private readonly SkillName[] m_AnimalFormRestrictedSkills = new SkillName[0];

        public override bool AllowSkillUse(SkillName skill)
        {
            // To allow ALL skills in Animal Form, we bypass the transformation check entirely.
            return DesignContext.Check(this);
        }
        ...
        private bool FindItems_Callback(Item item)
        {
            if (!item.Deleted && (item.LootType == LootType.Blessed || item.Insured))
            {
                ...
                {
                    // Don't eject blessed/insured items from blessed containers
                    if (item.Parent is Container parentContainer && parentContainer.LootType == LootType.Blessed)
                    {
                        return false;
                    }
                    return true;
