...
namespace Server.Engines.Fellowship
{
    ...
    public class Worker : BaseQuester
    {
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            if (from.Alive)
            {
                list.Add(new PurchaseFellowshipCoinEntry(from, this));
            }
        }

        private class PurchaseFellowshipCoinEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Worker m_Worker;

            public PurchaseFellowshipCoinEntry(Mobile from, Worker worker)
                : base(6103, 3) // "Purchase" cliloc with range of 3
            {
                m_From = from;
                m_Worker = worker;
            }
            ...
            {
                if (!m_From.Alive || !m_Worker.InRange(m_From.Location, 5))
                    return;

                const int coinPrice = 10000;

                if (Banker.Withdraw(m_From, coinPrice))
                {
                    FellowshipCoin coin = new FellowshipCoin();
                    m_From.AddToBackpack(coin);
                    m_Worker.SayTo(m_From, "Here is your Fellowship Coin. May it serve you well.");
                    m_From.SendMessage(0x3B2, "You purchase a Fellowship Coin for {0} gold.", coinPrice);
                }
                else
                {
                    m_Worker.SayTo(m_From, "You do not have enough gold in your bank. The Fellowship Coin costs {0} gold.", coinPrice);
                }
            }
        }
