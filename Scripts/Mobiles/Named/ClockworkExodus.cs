...
namespace Server.Mobiles
{
    ...
    public class ClockworkExodus : BaseCreature
    {
        ...
        private List<Mobile> m_Damagers = new List<Mobile>();
        ...
        {
            // Give ScrollofValiantCommendation to every player who damaged this creature
            foreach (Mobile damager in m_Damagers)
            {
                if (damager != null && !damager.Deleted)
                {
                    ScrollofValiantCommendation scroll = new ScrollofValiantCommendation();
                    DistributeArtifact(damager, scroll);
                }
            }

            // 20% chance for one random artifact to a random player
        ...
        public override void OnDamage(int amount, Mobile from, bool willKill)
        {
            ...
            // Track players who damage this creature
            if (from != null && from.Player && !m_Damagers.Contains(from))
            {
                m_Damagers.Add(from);
            }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // Version 1: Serialize damagers list
            writer.Write(m_Damagers.Count);
            foreach (Mobile damager in m_Damagers)
            {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version >= 1)
            {
                ...
                {
                    Mobile damager = reader.ReadMobile();
                    if (damager != null)
                    {
