...
*/
        ...
        */
            ...
            SkillName skill, Mobile m, int maxPointsToLearn, ref int pointsToLearn, bool doTeach)
        {
            ...
            // ↓ ADD
            bool isFreeSkill = SkillInfo.FreeSkills.Contains(skill);

            int freePoints = m.Skills.Cap - m.Skills.Total;
            int freeablePoints = 0;

            if (!isFreeSkill)  // ↓ WRAP this entire block
            {
                if (freePoints < 0)
                    freePoints = 0;
            ...
            if (doTeach)
            {
                ...
                /* Sanity check */
                if (baseToSet > theirSkill.CapFixedPoint ||
                    (!isFreeSkill && (m.Skills.Total - theirSkill.BaseFixedPoint + baseToSet) > m.Skills.Cap))
                {
