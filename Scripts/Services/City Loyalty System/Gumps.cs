...
namespace Server.Engines.CityLoyalty
{
    ...
    public class CityStoneGump : BaseGump
    {
        ...
        public override void AddGumpLayout()
        {
            // Governor system disabled. Show city stone info without governor or election content.
            AddBackground(0, 0, 400, 120, 5054);
            AddHtmlLocalized(10, 12, 380, 20, 1155758, 0xFFFF, false, false); // City Stone

            AddHtml(20, 45, 360, 20,
                String.Format("<basefont color=#FFFFFF>{0}", City.Definition.Name), false, false);

            AddHtml(20, 70, 360, 20,
                String.Format("<basefont color=#FFFFFF>Treasury: {0:N0} gp", City.Treasury), false, false);

            AddButton(10, 95, 4005, 4007, 0, GumpButtonType.Reply, 0);
            AddHtmlLocalized(45, 95, 250, 20, 1060675, 0xFFFF, false, false); // CLOSE
        }

        public override void OnResponse(RelayInfo info)
        {
            // All election/governor buttons removed; nothing to handle beyond close (ID 0).
        }
    }
    ...
    public class CityMessageGump : BaseGump
    {
        ...
        public override void AddGumpLayout()
        {
            ...
            if (City.PostedOn != DateTime.MinValue)
                AddHtmlLocalized(25, 85, 500, 20, 1154916, String.Format("{0}\t{1}", "City Administration", City.PostedOn.ToShortDateString()), 0, false, false); // Posted by ~1_NAME~ on ~2_date~
