            ...
            if (amount > 0)
            {
                ...
                if (scrollList.Count > 0)
                {
                    for (int i = 0; i < amount; i++)
                    {
                        ...
                        switch (random.Item1)
{
    // Case 1 was ScrollOfTranscendence. Now gives a Treasure Map of the current level.
    case 1: chest.DropItem(new TreasureMap(tMap.Level, chest.Map)); break; 
    
    // Case 2 was ScrollOfAlacrity. Now gives a Skeleton Key.
    case 2: chest.DropItem(new Skeletonkey()); break; 
    
    // Case 3 is PowerScrolls. We leave this alone so they still drop in Felucca.
