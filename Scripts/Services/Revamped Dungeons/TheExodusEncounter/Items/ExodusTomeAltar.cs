...
namespace Server.Items
{
    public class ExodusTomeAltar : BaseDecayingItem
    {
        ...
        // Removed CheckParty method – no longer needed

        public virtual void SendConfirmationsExodus(Mobile from)
        {
            // Solo: only the owner can begin the ritual
            if (from != this.Owner)
            {
                from.SendLocalizedMessage(1153610); // Only the altar owner can commence with the ritual.
                return;
            }

            // Check that the owner has the Robe of Rite equipped
            RobeofRite robe = from.FindItemOnLayer(Layer.OuterTorso) as RobeofRite;
            if (robe == null)
            {
                from.SendLocalizedMessage(1153609, from.Name); // ~1_PLAYER~ has not fulfilled all the requirements...
                return;
            }

            // Check that the owner has performed both parts of the ritual
            var entry = m_Rituals.FirstOrDefault(r => r.RitualMobile == from);
            if (entry == null || !entry.Ritual1 || !entry.Ritual2)
            {
                from.SendLocalizedMessage(1153609, from.Name);
                return;
            }

            // All checks passed – teleport the owner
            SendBattleground(from);
        }
        ...
        public virtual void SendBattleground(Mobile from)
        {
            if (VerLorRegController.Active && VerLorRegController.Mobile != null && ExodusSummoningAlter.CheckExodus())
            {
                ...
                // Teleport pets FIRST (while the player is still at the old location)
                BaseCreature.TeleportPets(from, this.m_TeleportDest, Map.Ilshenar);

                // Then move the player
                from.MoveToWorld(this.m_TeleportDest, Map.Ilshenar);

                // Delete the Robe of Rite
                RobeofRite robe = from.FindItemOnLayer(Layer.OuterTorso) as RobeofRite;
                if (robe != null)
                    robe.Delete();

                // Delete the altar after a short delay
                Timer.DelayCall(TimeSpan.FromSeconds(2), new TimerCallback(Delete));
            }
            else
            {
                from.SendLocalizedMessage(1075213); // The master of this realm has already been summoned...
            }
        }
    }
