...
namespace Server.Targets
{
    public class BladedItemTarget : Target
    {
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            // Changed from SwampDragon to IBardedMount
            else if (targeted is IBardedMount && targeted is BaseMount)
            {
                IBardedMount barded = (IBardedMount)targeted;
                BaseMount mount = (BaseMount)targeted;

                if (!barded.HasBarding)
                {
                    from.SendMessage("That mount is not wearing barding.");
                }
                else if (!mount.Controlled || mount.ControlMaster != from)
                {
                    from.SendLocalizedMessage(1053022); // You cannot remove barding from a mount you do not own.
                }
                else
                {
                    barded.HasBarding = false;
                    barded.BardingHP = 0;

                    from.SendMessage("You remove the barding from your mount.");
