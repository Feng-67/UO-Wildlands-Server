...
// COMMENT OUT THESE LINES to prevent Felucca from overriding your rules
			//var map = from.Map;
			...
			// Allow harm ONLY if they are at war (IsEnemy)
       			if (fromGuild.IsEnemy(targetGuild))
            		return true;
        
        		// Optional: Allow guildmates/allies to spar? 
        		// If not, don't include: || fromGuild == targetGuild || fromGuild.IsAlly(targetGuild)
			}

			// INSERT THE ARENA CHECK HERE
    if (from.Region.IsPartOf<Server.Engines.ArenaSystem.ArenaRegion>())
    {
        return true; // Allow them to fight if they are in an arena!
    }

    // --- FIX STARTS HERE ---
    // Only block combat if the TARGET is a Player or a Controlled Pet.
    // This allows you and your pets to attack monsters.
    if (target is PlayerMobile || (target is BaseCreature && ((BaseCreature)target).Controlled))
    {
    //from.SendMessage("PvP disabled except for Guild Wars or Dueling Arenas");
    return false;
}
