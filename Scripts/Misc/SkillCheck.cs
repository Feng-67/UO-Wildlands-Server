	...
	public class SkillCheck
	{
        ...
        public static void Gain(Mobile from, Skill skill, int toGain)
		{
			...
			if (skill.Base < skill.Cap && skill.Lock == SkillLock.Up)
			{
                ...
                if (from is PlayerMobile && !SkillInfo.FreeSkills.Contains(skill.SkillName))
                {
                    CheckReduceSkill(skills, toGain, skill);
                }

                if (!from.Player || SkillInfo.FreeSkills.Contains(skill.SkillName) || (skills.Total + toGain <= skills.Cap))
                {
