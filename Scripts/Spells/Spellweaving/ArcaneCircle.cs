        ...
        public override void OnCast()
{
    if (CheckSequence())
    {
        ...
        // 1. Calculate Duration: Max skill bonus + 6 hours (the max group bonus)
        TimeSpan duration = TimeSpan.FromHours(Math.Max(1, (int)(Caster.Skills.Spellweaving.Value / 24)));
        duration += TimeSpan.FromHours(6); 

        // 2. Calculate Strength: Set to 6
        int strengthBonus =  6;

        // This still loops through anyone nearby so they get the max bonus too
