    ...
    public class Spellbook : Item, ICraftable, ISlayer, IEngravable, IVvVItem, IOwnerRestricted, IWearableDurability, IFactionItem
	{
		...
		public static Spellbook FindSpellbookInList(List<Spellbook> list, Mobile from, int spellID, SpellbookType type)
		{
			...
			for (int i = list.Count - 1; i >= 0; --i)
			{
				...
				// We changed 'book.Parent == pack' to 'book.IsChildOf(pack)'
if (!book.Deleted && (book.Parent == from || (pack != null && book.IsChildOf(pack))) &&
    ValidateSpellbook(book, spellID, type))
		...
		public static List<Spellbook> FindAllSpellbooks(Mobile from)
		{
			...
			// 'true' tells the server to search recursively inside all sub-containers (like your strap)
list.AddRange(pack.FindItemsByType<Spellbook>(true));
...
// "IsChildOf" checks the backpack and any bag inside it (recursively)
if (Parent == from || (pack != null && IsChildOf(pack)))
{
    DisplayTo(from);
}
else
